...
export const scheduleMedicationReminder = async (title: string, body: string, hour: number, minute: number) => {
  ...
  console.log(`Notifikasi dijadwalkan pada ${trigger.toLocaleString('id-ID')} dengan ID: ${id}`);
  return id;
};
...
export const scheduleMultipleReminders = async (
  ...
  await AsyncStorage.setItem(`reminder_group_${title}_${Date.now()}`, JSON.stringify(ids));
  return ids;
};
...
export const deleteScheduleMedicatioReminderForever = async (id: string) => {
  await Notifications.cancelScheduledNotificationAsync(id);
  await AsyncStorage.removeItem(`reminder_${id}`);
};